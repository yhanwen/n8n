		...
		workflowExecuteAfter: [
			async function (
				...
				try {
					if (
						!isManualMode &&
						WorkflowHelpers.isWorkflowIdValid(this.workflowData.id as string) === true &&
						newStaticData
					) {
						...
						try {
							...
							Logger.error(
								`There was a problem saving the workflow with id "${this.workflowData.id}" to save changed staticData: "${error.message}" (hookFunctionsSave)`,
					...
					if (
						...
						WorkflowHelpers.isWorkflowIdValid(this.workflowData.id.toString()) === true
					) {
				...
				try {
					if (
						...
						try {
							...
							Logger.error(
								`There was a problem saving the workflow with id "${this.workflowData.id}" to save changed staticData: "${error.message}" (workflowExecuteAfter)`,
